...
class SWTForward(nn.Module):
    ...
    def __init__(self, J=1, wave='db1', mode='symmetric',requires_grad= False):
        super().__init__()
        wave_type = wave
        ...
        h0_col = np.array(h0_col)*math.sqrt(2)/2
        h1_col = np.array(h1_col)*math.sqrt(2)/2
        h0_row = np.array(h0_row)*math.sqrt(2)/2
        h1_row = np.array(h1_row)*math.sqrt(2)/2
        # if wave_type == 'db1':
        #     h0_col = [0.5,0.5]
        #     h1_col = [-0.5, 0.5]
        #     h0_row = [0.5, 0.5]
        #     h1_row = [-0.5, 0.5]
        print(h0_col, h1_col, h0_row, h1_row)
        # Prepare the filters
        filts = prep_filt_afb2d(h0_col, h1_col, h0_row, h1_row)
        self.h0_col = nn.Parameter(filts[0], requires_grad=requires_grad)
        self.h1_col = nn.Parameter(filts[1], requires_grad=requires_grad)
        self.h0_row = nn.Parameter(filts[2], requires_grad=requires_grad)
        self.h1_row = nn.Parameter(filts[3], requires_grad=requires_grad)
...
class SWTInverse(nn.Module):
    ...
    def __init__(self, wave='db1', mode='symmetric',requires_grad=False):
        super().__init__()
        wave_type = wave
        if isinstance(wave, str):
            wave = pywt.Wavelet(wave)
        ...
        #     g0_row = [1, 1]
        #     g1_row = [1, -1]
        g0_col = np.array(g0_col)*math.sqrt(2)
        g1_col = np.array(g1_col)*math.sqrt(2)
        g0_row = np.array(g0_row)*math.sqrt(2)
        g1_row = np.array(g1_row)*math.sqrt(2)
        print(g0_col, g1_col, g0_row, g1_row)
        filts = prep_filt_sfb2d(g0_col, g1_col, g0_row, g1_row)
        self.g0_col = nn.Parameter(filts[0], requires_grad=requires_grad)
        self.g1_col = nn.Parameter(filts[1], requires_grad=requires_grad)
        self.g0_row = nn.Parameter(filts[2], requires_grad=requires_grad)
        self.g1_row = nn.Parameter(filts[3], requires_grad=requires_grad)
...
class DWTForward(nn.Module):
    ...
    Args:
        ...
        mode (str): 'zero', 'symmetric', 'reflect' or 'periodization'. The
            padding scheme
        """
    def __init__(self, J=1, wave='db1', mode='zero',requires_grad= False):
        super().__init__()
        if isinstance(wave, str):
            wave = pywt.Wavelet(wave)
        if isinstance(wave, pywt.Wavelet):
...
class DWTInverse(nn.Module):
    ...
    Args:
        ...
        mode (str): 'zero', 'symmetric', 'reflect' or 'periodization'. The
            padding scheme
    """
    def __init__(self, wave='db1', mode='zero',requires_grad= False):
        super().__init__()
        if isinstance(wave, str):
            wave = pywt.Wavelet(wave)
        if isinstance(wave, pywt.Wavelet):
